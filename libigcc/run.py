...
def run_compile(subs_compiler_command, runner):
    ...
    stdoutdata, stderrdata = compile_process.communicate(
        bytes(source_code.get_full_source(runner), encoding="utf-8")
    )
...
def run_exe(exefilename):
    run_process = subprocess.Popen(
        exefilename, stdout=subprocess.PIPE, stderr=subprocess.PIPE
    )
...
class UserInput:
    ...
    def __eq__(self, other):
        return (
            ...
            and self.output_chars == other.output_chars
            and self.error_chars == other.error_chars
        )
...
def run(outputfile=sys.stdout, inputfile=None, print_welc=True, argv=None):
